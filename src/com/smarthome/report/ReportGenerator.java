...
public class ReportGenerator {
    ...
    // Use Locale.US everywhere so decimal separator is always '.' regardless of
    // the system locale (e.g. Romanian locale uses ',' which breaks assertions).
    ...
    public static String generateSummaryReport(SmartHome home) {
        ...
        long totalSensors = home.getRooms().stream()
                .mapToLong(r -> r.getSensors().size())
                .sum();
        sb.append("Total rooms  : ").append(home.getRooms().size()).append("\n");
        sb.append("Total sensors: ").append(totalSensors).append("\n\n");

        Map<AlertLevel, Long> alertCounts = home.getRooms().stream()
                .flatMap(r -> r.getSensors().stream())
                .collect(Collectors.groupingBy(Sensor::checkAlertLevel, Collectors.counting()));

        sb.append("Alert breakdown:\n");
        for (AlertLevel level : AlertLevel.values()) {
            sb.append(String.format(REPORT_LOCALE, "  %-10s: %d%n",
                    level, alertCounts.getOrDefault(level, 0L)));
        }
        return sb.toString();
    }
